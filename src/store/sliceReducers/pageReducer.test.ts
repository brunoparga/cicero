...
it('sets a wrong guess', () => {
  const page = JSON.parse(JSON.stringify(basePage));
  const action = { type: 'SET_GUESS', payload: { property: 'rightGuess', value: false } };
  const expectation = { ...page, guess: { ...page.guess, rightGuess: false } } as PageState;
  expect(pageReducer(page, action)).toEqual(expectation);
});

it('sets a right guess that makes all the guesses right', () => {
  const page = JSON.parse(JSON.stringify(basePage));
  const action = { type: 'SET_GUESS', payload: { property: 'wrongGuess', value: true } };
  const expectation = {
    ...page,
...
it('sets another kind of right guess', () => {
  ...
  const action = { type: 'SET_GUESS', payload: { property: 'thirdGuess', value: true } };
  const expectation = { ...newPage, guess: { ...newPage.guess, thirdGuess: true } } as PageState;
  expect(pageReducer(page, action)).toEqual(expectation);
});
