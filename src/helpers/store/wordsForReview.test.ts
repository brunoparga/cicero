...
it('sets 1/3 of words for English to Latin translation', () => {
  ...
  expect(wordsForReview(word)).toEqual({ ...word, questionType: QuestionType.Translation });
});

it('sets words as indeclinable if they have the relevant property', () => {
  jest.spyOn(global.Math, 'random').mockReturnValue(0.5);
  const word = { properties: { indeclinable: true } } as Word;
  expect(wordsForReview(word)).toEqual({ ...word, questionType: QuestionType.Indeclinable });
});
...
it('sets numerals and pronouns as "adjectives" if they decline as such', () => {
  ...
  expect(wordsForReview(word1)).toEqual({ ...word1, questionType: QuestionType.Adjective });
  expect(wordsForReview(word2)).toEqual({ ...word2, questionType: QuestionType.Adjective });
});
